...
public final class LatinIME extends InputMethodService implements KeyboardActionListener,
	...
	private Key findNearestKey(int direction,Key srcKey){
		...
		switch(direction){
		case UP:
			nearestKeyIndices = mCurrentKeyboard.getNearestKeys(srcKey.mX,srcKey.mY);
			for(i = 0; i < nearestKeyIndices.length; i++){
				Key nearKey = nearestKeyIndices[i];
				distY = srcKey.mY - nearKey.mY;
				if(distY >= nearKey.mHeight){
					distX = abs(nearKey.mX - srcKey.mX);
					if((distY < tmpY) ||
		...
		case DOWN:
			...
			for(i = 0; i < nearestKeyIndices.length; i++){
				Key nearKey = nearestKeyIndices[i];
				distY = nearKey.mY - srcKey.mY;
				if(distY >= srcKey.mHeight){
					distX = abs(nearKey.mX - srcKey.mX);
					if((distY < tmpY) ||
						(distY == tmpY && distX <= tmpX)){
						retKey = nearKey;
		...
		case LEFT:
			for(i = 0; i < mKeys.length; i++){
				Key nearKey = mKeys[i];
				int dY = nearKey.mY - srcKey.mY;
				if((dY >= (0 - accuracy)) && (dY <= accuracy)){
					distX = srcKey.mX - nearKey.mX;
					if(distX >= nearKey.mWidth && distX <= tmpX){
						retKey= nearKey;
						tmpX = distX;
					}
				}
		...
		case RIGHT:
			for(i = 0; i < mKeys.length; i++){
				...
				int dY = nearKey.mY - srcKey.mY;
				if((dY >= (0 - accuracy)) && (dY <= accuracy)){
					distX = nearKey.mX - srcKey.mX;
					if(distX >= srcKey.mWidth && distX <= tmpX){
						retKey= nearKey;
						tmpX = distX;
					}
				}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event){
		...
		case KeyEvent.KEYCODE_DPAD_CENTER:
			...
			mPreKey = mLastKey;
			MotionEvent me = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_DOWN, mLastKey.mX, mLastKey.mY, 0);
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        case KeyEvent.KEYCODE_DPAD_CENTER:
        	 if(!setFields())
                 break;
			 mPreKey = null;
			 MotionEvent me = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_UP, mLastKey.mX, mLastKey.mY, 0);
			 mInputView.onTouchEvent(me);
			 break;
        }
